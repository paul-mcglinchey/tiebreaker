...
const AddNewClientForm = ({ userGroup, setIsLoading, setSuccess }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    await fetch((endpoints.clients), requestHelper.requestBuilder('POST', values))
      .then(res => {
        if (res.ok) setSuccess('Successfully added client');
        setIsLoading(false);
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    ...
    >
      {({ errors, touched }) => (
        <Form className="flex flex-grow flex-col space-y-3 content-end">
