...
import { endpoints, generateColour, profileColours, requestHelper } from "../../../utilities";
import { ApplicationContext } from "../../../utilities/contexts";

const ColourPicker = ({ client }) => {
  ...
  const updateProfileColour = async (clientColour) => {
    setStatus({
      isLoading: true,
      success: '',
      error: ''
  ...
  await fetch((endpoints.colours(client && client._id)), requestHelper.requestBuilder('PUT', { clientColour: clientColour }))
    .then(res => {
      if (res.ok) {
